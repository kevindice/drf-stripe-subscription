...
def _get_or_create_stripe_user_from_user_pk_email(user_pk, user_email: str, customer_id: str = None):
    ...
    """
    if customer_id:
        stripe_user, created = StripeUser.objects.get_or_create(user_id=user_pk, customer_id=customer_id)
    else:
        stripe_user, created = StripeUser.objects.get_or_create(user_id=user_pk)
